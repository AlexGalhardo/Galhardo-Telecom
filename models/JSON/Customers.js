		...
		try {
			for (let i = 0; i < database.customers.length; i++) {
				if (
					...
					&&
					database.customers[i].confirm_email_token === token) {
		...
		try {
			...
			for (let i = 0; i < database.customers.length; i++) {
				...
				if (database.customers[i].id === SESSION_USER.id) {
					...
					const olderPasswordValid = await Bcrypt.comparePassword(userObject.older_password, database.customers[i].password)
					...
					if (userObject.street_number) database.customers[i].address.street_number = userObject.street_number
